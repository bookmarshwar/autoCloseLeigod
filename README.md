...
| [策略2: 键鼠检测](#策略2-键鼠空闲守护) | **独立运行**: 按键鼠空闲探测自动暂停, 与策略0/1 互不依赖 | 关 |
...
| `standalone.listenSeconds` | 3 | 每次探测的监听窗口(秒) |
| `standalone.idleMinutes` | 15 | 连续无键鼠活动的暂停阈值(分钟) |
| `standalone.probeIntervalSeconds` | 30 | 探测间隔(秒), 与策略0 轮询间隔无关 |

> **策略2 不依赖策略0/1**: 只要 `enabled` 就独立运行空闲探测, 与其他策略并行;
> 探测失败视为有活动(防误暂停); 任何策略触发暂停后统一进入低频休眠(见「通用机制」)。
