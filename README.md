...
## 策略(config.json 的 `strategies` 段, 全部可配置, 各自独立开关)

看门狗的全部行为都是策略, **三个策略可独立启用/关闭**:

### 策略0: 主流程(默认开, 开箱即用的轮询守护)

```json
"strategy0": { "enabled": true }
```

轮询时长状态 → 询问游戏 → 复查 → 进程检查 → 暂停时长(即上方「工作流程」全部逻辑)。
- 关闭后: 不再轮询/守护/自动暂停, 看门狗只做其余已启用的策略;
  若策略1 也未启用 → 没有任何策略时看门狗直接退出。
- 注意: 策略2(键鼠检测)依附于主流程的关闭判断, 主流程关闭时策略2 不生效。
- 用 `--strategy0` 参数可临时开启。
...
| `strategies` | 见上方「策略」 | 策略0 主流程 / 策略1 定时关闭 / 策略2 键鼠检测延后, 各自独立开关 |
